(function(){
  ...
  function deptApprovalTable(){ const rows=deptReviewRows(); if(!rows.length) return '<div class="empty">لا توجد بنود مسجلة لهذا الأوردر في القسم حتى الآن.</div>'; const total=rows.reduce((s,r)=>s+rowSale(r)*rowQty(r),0); return table(rows,['الحالة','الأوردر','البند','كمية','سعر','إجمالي'],r=>[esc(rowBillingStatus(r)||rowCloseStatus(r)||'قيد مراجعة القسم'),esc(rowOrderId(r)),esc(rowItem(r)),esc(rowQty(r)),money(rowSale(r)),money(rowSale(r)*rowQty(r))])+'<div class="softBox"><b>إجمالي مسودة القسم: '+money(total)+'</b></div>'; }
  function screenDept(){
  ...
  window.ES27 = {
    ...
    collectDeptLines(){ const order=val('fiOrder'); const rows=(state.data.deptLines||[]).filter(isUnbilledDeptLine).filter(isDeptApprovedForFinal).filter(r=>String(rowOrderId(r)||'')===String(order||'')); const total=rows.reduce((s,r)=>s+num(r.sale)*num(r.qty||1),0); const b=$('finalBox'); if(b) b.innerHTML=table(rows,['القسم','البند','كمية','سعر'],r=>[esc(r.department),esc(r.itemName),esc(r.qty),money(r.sale)])+'<div class="softBox"><b>الإجمالي: '+money(total)+'</b></div>'; },
    saveFinal(){ const order=val('fiOrder'); const rows=(state.data.deptLines||[]).filter(isUnbilledDeptLine).filter(isDeptApprovedForFinal).filter(r=>String(rowOrderId(r)||'')===String(order||'')); const total=rows.reduce((s,r)=>s+num(r.sale)*num(r.qty||1),0); const p={orderId:order,customer:val('fiCustomer'),total,paid:num(val('fiPaid')),remain:Math.max(0,total-num(val('fiPaid'))),date:new Date().toISOString()}; state.data.finalInvoices.unshift(p); saveLocal(); api('saveAccountingFinalInvoice',p).catch(()=>{}); shell(); flash('تم تقفيل الفاتورة'); },
